...
* On a `PizzaBakeFinishedEvent`, retrieve the associated `KitchenOrder`. If the `KitchenOrder` is baking, `startAssembly()`; otherwise, retrieve all of the associated `Pizza` instances. If all of them have finished baking, `finishAssembly` on the `KitchenOrder`.
